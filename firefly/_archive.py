...
def _search_all(exoplanet):
    ...
    exo_list = (
        exo_archive[["pl_name"]].dropna().drop_duplicates("pl_name").values.tolist()
    )
    exo_list = [j for i in exo_list for j in i]
    exo_list = natsorted(exo_list)
...
def _pl(tic_id):
    ...
    """
    df = exo_nasa[["pl_name", "tic_id"]].dropna().sort_values("pl_name")
...
def tess_candidate():
    ...
    for i in range(13, 21):
        ...
        df = (
            df.groupby(df.links.str[-30:-15].astype(int))
            .size()
...
def priors(exoplanet, archive="eu", save=False, user=True, auto=True, fit_ttv=False):
    ...
    if archive == "all":
        ...
        exo_archive["pl_name"] = Categorical(
            exo_archive["pl_name"],
            ordered=True,
            categories=natsorted(exo_archive["pl_name"].unique()),
        )
        exo_archive = (
            exo_archive.sort_values("pl_name")
            .set_index("pl_name")
            .drop(["Archive"], axis=1)
        )
    ...
    try:
        ...
        sys.exit(
            "The chosen target is either spelt incorrectly, or does not "
    ...
    cols = [
        [
            "P",
            ["fixed" if archive == "spearnet" else "gaussian"][0],
            P,
        ...
        [
            ...
            "gaussian",
            i,
            [i * 2e-1 if archive == "spearnet" else i * 2e-1][0],
            "",
        ],
        [
            "w",
            ["fixed" if (w == 90 or ecc == 0) else "gaussian"][0],
        ...
        [
            "ecc",
            ["fixed" if (ecc == 0 or w == 90) else "gaussian"][0],
            [0 if w == 90 else ecc][0],
            [
                ""
                if (ecc == 0 or w == 90)
                else ecc * 0.5
        ...
        [
            "rp",
            ["uniform" if archive == "spearnet" else "uniform"][0],
            [0.5 * rp if archive == "spearnet" else (0.5 * radius_const * rp / rs)][0],
            [1.5 * rp if archive == "spearnet" else (1.5 * radius_const * rp / rs)][0],
            0,
        ],
    ]
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
    # Save the priors
    priors_csv = DataFrame(
        cols, columns=["Parameter", "Distribution", "Input_A", "Input_B", "Filter"]
    )
    if save == True:
    ...
    repack = DataFrame(
        cols, columns=["Parameter", "Distribution", "Input A", "Input B", "Filter"]
    )
...
def gen_tess(archive="nasa"):
    ...
    df = DataFrame(data)
    df["Exoplanet"] = Categorical(
        df["Exoplanet"], ordered=True, categories=natsorted(df["Exoplanet"].unique())
    )
...
def gen_tess_ttv():
    ...
    for i, exoplanet in enumerate(ttv_list):
        try:
            ...
            pass
    data = {
        "Exoplanet": viable_ttv,
        "Products": products_ttv,
        "Period": period_ttv,
        "Epochs": epochs_ttv,
    }
    df = DataFrame(data)
