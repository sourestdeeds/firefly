...
def plot_epoch(sub=False):
    ...
    for i, archive in enumerate(archive_list):
        ...
        df = (
            pd.read_csv(f"{here}/data/Targets/{archive}_tess_viable.csv")
        ...
        candidates = (
            f"{len(df)} {archive.upper()} Archive "
        ...
        highlights = df.groupby(
            pd.cut(df["Epoch Frequency"], np.arange(0, 1.0 + 0.1, 0.1))
        ).sum()
        ...
        ax = plt.subplot(temp)
        [
            ax.axvline(x=i + 0.4, color="k", marker=",", alpha=0.5, ymin=0, ymax=0.15)
            for i in range(10)
        ]
        [
            ax.axvline(x=i + 0.45, color="k", marker=",", alpha=0.5, ymin=0.8, ymax=1)
            for i in range(10)
        ]
        ...
        [
            ax.text(
                i + 0.34,
                total_epochs / 47,
                f"{i+1}0% of All Transits - {round(cumsum_cand[i]*100/total_candidates)}% Candidates",
                rotation=90,
            )
            for i in range(10)
        ]
        [
            ax.text(
                i - 0.1,
                highlights["Epochs"][i] + 5,
                str(highlights[candidates][i]),
                rotation=0,
            )
            for i in range(10)
        ]
        ...
        column_labels = [
            f"{i}0 - {i+1}0%".replace("00 - 10%", "0 - 10%") for i in range(10)
        ]
...
def lc_plot(file, flatten=False):
    ...
    pg = lc.to_periodogram(
        oversample_factor=25, maximum_period=period * 1.25, minimum_period=period * 0.75
    )
    # PLOT!
    ...
    plt.scatter(folded_lc.time, folded_lc.flux, alpha=0.4, color="k", zorder=2, s=2)
    plt.xlabel("Phase")
    plt.ylabel("Flux")
...
def mw():
    ...
    plot_instance.mw_scatter(
        -c.galactic.cartesian.x, c.galactic.cartesian.y, [z, "Transits Observed"]
    )
    ...
    plot_instance.scatter(
        -c2.galactic.cartesian.x, c2.galactic.cartesian.y, color="dimgrey", zorder=1
    )
    plot_instance.mw_scatter(
        -c.galactic.cartesian.x,
...
def oc(t0, t0_err, file="Complete_Results.csv", exoplanet=None):
    path_ttv = file
    data_ttv = (
        pd.read_csv(path_ttv)
        .set_index("Parameter")
        .filter(like="t0", axis=0)
        .drop(["Telescope", "Filter"], axis=1)
    )
    ...
    for (level, i) in zip(false_alarm_levels, levels):
        ...
        ls_ax.annotate(
            f"{str(int(i*100))}" + "$\%$", (pos, level * 1.018), color="k", ha="center"
        )
    ls_ax.annotate(
        f"Period: {int(period)}\n{fap*100:.2f}$\%$",
        (period, power.max() * 1.03),
        color="k",
        weight="bold",
        ha="center",
    )
    ...
    if exoplanet == None:
        ...
        fig.savefig(
            f"firefly/{exoplanet}/{exoplanet.lower().replace(' ', '')}_o-c.jpg",
            bbox_inches="tight",
        )
...
def oc_fold(t0, t0err, P, file="Complete_results.csv", exoplanet=None, longterm=True):
    ...
    t0_cerr = data_ttv["Error"].astype(float).values
    epoch_no = (
        np.array([find_epoch_no(i, t0_o, P, converter) for i in range(len(t0_c))]) * P
    )
    ...
    if longterm == True:
        ...
        frequency, power = ls.autopower(
            nyquist_factor=1, samples_per_peak=25, minimum_frequency=1 / max_p
        )
    ...
    txt = AnchoredText(
        f"$\chi^2_{{{red}}} = {chi2_red:.2f}\, ({nsig:.2f}\sigma)$"
        + f"\n$\mu_{{error}}= {loss:.2f}$",
        loc="upper right",
        frameon=False,
    ...
    phase_ax.scatter(epoch_phase, ominusc, c=epoch_no, marker=".", zorder=2, alpha=0.5)
    phase_ax.axhline(0, color="black", linestyle="--", linewidth=1)
    phase_ax.plot(
        fit_x_phase[np.argsort(fit_x_phase)],
        fit_y[np.argsort(fit_x_phase)],
        color="red",
        alpha=0.8,
    )
    ...
    if exoplanet == None:
        ...
        if longterm == True:
            fig.savefig(
                f"firefly/{exoplanet}/{exoplanet} o-c longterm.jpg", bbox_inches="tight"
            )
        else:
            fig.savefig(f"firefly/{exoplanet}/{exoplanet} o-c.jpg", bbox_inches="tight")
...
def read_fitted_lc(exoplanet, transits):
    ...
    # Read in lc's
    df = pd.DataFrame(
        columns=(["Phase", "Normalised flux", "Flux uncertainty", "Best fit curve"])
    )
    for i in range(0, epoch_no):
        file = f"firefly/{exoplanet}/fitted_lightcurves/t0_f0_e{i}_detrended.csv"
        lc = pd.read_csv(file)[
            ["Phase", "Normalised flux", "Flux uncertainty", "Best fit curve"]
        ]
    ...
    transit_mask = np.ma.masked_values(fit_combined, 1.0).mask
    ...
    try:
        if (
            (transit_loc[0][0] + window > len(fit_combined))
    ...
    if fluxdiff < 0.001:
        ...
        sigma = 6
    df_transit_mask["Residual"] = (
        df_transit_mask["Normalised flux"] - df_transit_mask["Best fit curve"]
    )
    clip = sigma_clip(df_transit_mask["Residual"], sigma=sigma, stdfunc=mad_std).mask
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
    # Apply Mask
...
def density_scatter(exoplanet, transits, P, cadence):
    ...
    time_all, flux_all, flux_err_all, fit_xall, fit_yall = read_fitted_lc(
        exoplanet, transits
    )
    ...
    # Residuals
    res_ax.errorbar(
        x, diff, yerr, color="dimgrey", alpha=0.1, zorder=1, capsize=2, ls="none"
    )
    ...
    plt.set_cmap("hot")
    # Light Curve
    ax.errorbar(x, y, yerr, color="dimgrey", alpha=0.1, zorder=1, capsize=2, ls="none")
    ...
    hist_ax.hist(
        bins[:-1],
        bins,
        weights=weighted_binned_counts,
        orientation="horizontal",
        color=facecolor,
        edgecolor=rgba_color,
        histtype="stepfilled",
    ...
    fig[0].savefig(f"firefly/{exoplanet}/{exoplanet} density.png", bbox_inches="tight")
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
    # No errorbar
    ...
    ax.scatter(
        binned_phase, binned_flux, zorder=3, s=5, color="white", edgecolor="none"
    )
    ax.add_artist(txt)
    # Histogram
    rgba_color = colors.to_rgba("dimgrey")
    ...
    plt.subplots_adjust(wspace=0.0)
    fig[0].savefig(
        f"firefly/{exoplanet}/{exoplanet} density noerr.png", bbox_inches="tight"
    )
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
    # No resid or err
    sns.set_theme(style="darkgrid")
    ...
    ax.set_ylabel("Normalised Flux")
    fig[0].savefig(
        f"firefly/{exoplanet}/{exoplanet} density noresiderr.png", bbox_inches="tight"
    )
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
    # No resid
    sns.set_theme(style="whitegrid")
    ...
    ax.scatter(x, y, c=z, zorder=2, s=5, edgecolor="none")
    ax.scatter(
        binned_phase, binned_flux, zorder=4, s=5, color="white", edgecolor="none"
    )
    ax.plot(fit_xall, fit_yall, marker="", color="k", zorder=3, lw=1)
    ...
    ax.set_ylabel("Normalised Flux")
    fig[0].savefig(
        f"firefly/{exoplanet}/{exoplanet} density noresid.png", bbox_inches="tight"
    )
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
...
def plot_coupled_comparison():
    ...
    uncoupled = (
        pd.read_csv(f"{here}data/spear_uncoupled.csv")
        .set_index("pl_name")
        .drop("WD 1856+534 b", axis=0)
    )
    ...
    uncoupled = (
        pd.read_csv(f"{here}data/spear_uncoupled.csv")
        .set_index("pl_name")
        .drop(missing_uncoup, axis=0)
        .drop("WD 1856+534 b", axis=0)
    )
    uncoupled = uncoupled.reset_index()
    coupled = (
        pd.read_csv(f"{here}data/spear.csv")
        .set_index("pl_name")
        .drop(missing_coup, axis=0)
    )
...
def plot_ld_params():
    ...
    uncoupled = (
        pd.read_csv(f"{here}spear_uncoupled_ld_params.csv")
        .set_index("pl_name")
        .drop("WD 1856+534 b", axis=0)
    )
    uncoupled = uncoupled.reset_index()
    ...
    uncoupled = uncoupled.reset_index()
    coupled = (
        pd.read_csv(f"{here}spear_coupled_ld_params.csv")
        .set_index("pl_name")
        .drop(missing_coup, axis=0)
    )
    coupled = coupled.reset_index()
    ...
    txt = AnchoredText(
        f"$R=${r:.2f}", frameon=False, loc="lower right", prop=dict(fontweight="bold")
    )
    q0.add_artist(txt)
