...
def _TESS_filter():
    ...
    df = df.append([{"filter_idx": 0, "low_wl": tess_filter}], ignore_index=True)
...
def mast(exoplanet):
    ...
    search = obs.query_criteria(
        dataproduct_type=["timeseries"], project="TESS", target_name=tic_id
    ).to_pandas()
    data = search[["obs_id", "target_name", "t_exptime", "provenance_name", "project"]]
    data = data.rename(
        columns={
            "obs_id": "Product",
            "target_name": "TIC ID",
    ...
    data["Product"] = Categorical(
        data["Product"], ordered=True, categories=natsorted(data["Product"].unique())
    )
...
def _discover_search(tic_id):
    ...
    data = search[
        ["obs_id", "target_name", "dataURL", "t_exptime", "provenance_name", "project"]
    ]
    data["dataURL"] = [
        "https://mast.stsci.edu/api/v0.1/Download/file/?uri=" + data["dataURL"][i]
    ...
    for j, fitsfile in enumerate(lc_links):
        ...
        for i in range(len(time)):
            write_dict.append(
                {"Time": time[i], "Flux": flux[i], "Flux err": flux_err[i]}
            )
...
def _fits(exoplanet, exo_folder, cache, hlsp, cadence, bitmask):
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
    # MAST Search
    tic_id = _tic(exoplanet).replace("TIC ", "")
    print(f"\nSearching MAST for {exoplanet} (TIC {tic_id}).")
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
    # Print search result
    show = data[["obs_id", "target_name", "t_exptime", "provenance_name", "project"]]
    show = show.rename(
        columns={
            "obs_id": "Product",
            "target_name": "TIC ID",
    ...
    show["Product"] = Categorical(
        show["Product"], ordered=True, categories=natsorted(show["Product"].unique())
    )
    ...
    for j, fitsfile in enumerate(lc_links):
        ...
        TESS_fits.write(f"{source}/{mast_name}/{mast_name}.fits", overwrite=True)
        ...
        DEFAULT_BITMASK = [
            ...
            ImpulsiveOutlier,
            Argabrightening,
            BadCalibrationExclude,  # , Straylight2
        ]
        HARD_BITMASK = [
            AttitudeTweak,
            SafeMode,
...
def _retrieval(
    ...
    if archive == "nasa":
        ...
        host_T, host_z, host_r, host_logg, t0, P, t14, repack = priors(
            exoplanet,
            archive=archive,
            save=True,
            user=False,
    ...
    for i, csvfile in enumerate(csv_in_dir):
        split_curves = split_lightcurve_file(
            csvfile, t0=t0, P=P, t14=t14, cutoff=cutoff, window=window
        )
    ...
    try:
        copy(
            f"{exo_folder}/plots/folded_curves/with_errorbars/filter_0.png",
    ...
    # Save Best Values
    master = read_csv(
        f"{exo_folder}/output_parameters/Complete_results.csv", index_col="Parameter"
    )
    ...
    try:
        ...
        werr = float()
    try:
        mad, madbin, obs_depth, cadences = density_scatter(
            exoplanet=exoplanet, transits=int(len(df)), P=P, cadence=cadence
        )
        copy(
            f"{exo_folder}/{exoplanet} density.png",
            f"firefly/plots/density/{exoplanet}.png",
        )
        copy(
            f"{exo_folder}/{exoplanet} density noerr.png",
    ...
    if fit_ttv == True:
        try:
            ...
            chi2_redlong, nsiglong, losslong, faplong, periodlong = oc_fold(
                t0ttv, t0errttv, file=file, P=P, exoplanet=exoplanet
            )
            try:
                chi2_red, nsig, loss, fap, period = oc_fold(
                    t0ttv, t0errttv, file=file, P=P, exoplanet=exoplanet, longterm=False
                )
            except:
                chi2_red, nsig, loss, fap, period = float(), float(), float(), float()
                pass
            copy(
                f"{exo_folder}/{exoplanet} o-c longterm.jpg",
                f"firefly/plots/oc/{exoplanet} long.jpg",
            )
            copy(
                f"{exo_folder}/{exoplanet} o-c.jpg", f"firefly/plots/oc/{exoplanet}.jpg"
            )
            data = {
                "pl_name": exoplanet,
                "red_chi2": chi2_red,
                ...
                "o-c_period": period,
                "faplong": faplong,
                "o-c_periodlong": periodlong,
                "pl_orbper": P,
                "pl_orbpererr1": Perr,
                "pl_tranmid": t0,
                "pl_tranmiderr1": t0err,
                "pl_orbsmax": a,
                "pl_orbsmaxerr1": aerr,
                "pl_radj": rp,
                "pl_radjerr1": rperr,
                "pl_orbincl": inc,
                "pl_orbinclerr1": incerr,
                "pl_orbeccen": ecc,
                "pl_orbeccenerr1": eccerr,
                "pl_orblper": w,
                "pl_orblpererr1": werr,
                "q0": q0,
                "q0err": q0err,
                ...
                "Unbinned Sigma": mad,
                "Binned Sigma": madbin,
                "Transit Depth": t_depth,
                "Sensitivity": sens,
                "Binned Sensitivity": sens_bin,
                "Observation Depth": obs_depth,
                "Observation Sensitivity": obs_sens,
                "Binned Observation Sensitivity": obs_sens_bin,
                "Total Cadences": cadences,
            }
...
def spearnet_archive_ld_params(source="13_April_2021_folded_uncoupled"):
    ...
    for i, zf in enumerate(zip_in_dir):
        with zipfile.ZipFile(zf) as zip:
            ...
            with zip.open(
                f"{exoplanet}/output_parameters/Complete_results.csv"
            ) as myZip:
